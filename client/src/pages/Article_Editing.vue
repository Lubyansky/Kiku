<template>
    <div class = "editing" v-cloak>
      ...
      <form class = "editing__container" v-else-if = "(this.permission.canEditArticles || this.permission.canAddArticles) && !this.is_banned">
          ...
          <div class = "general-information__preview">
              <site-input
                :value = "article.info.preview"
                @input = "article.info.preview = $event.target.value"
                type = "text" 
                placeholder="Добавьте ссылку или загрузите изображение (Кратно размеру 478*268)"
              />
              <label for="preview_file" class="general-information__input-file_label disabled">Обзор</label>
              <input class="general-information__input-file"  disabled="disabled"
                id="preview_file" 
                ref="file"
                type="file" 
                accept="image/*"
          ...
          <div class = "editing__content">
            ...
            <div class = "editing__content-item" v-for = "(item, index_c) in article.content" :key = "index_c">
              <remove-item-button @click.prevent="RemoveContentItem(index_c)" title="Удалить главу"></remove-item-button>
              <div class = "editing__content-item-container">
                <site-input class = "first-input"
              ...
              <div class = "editing__content-item-container">
                <add-item-button @click.prevent="AddTextItem(index_c)" :hasNoBorder = "true" title = "Добавить параграф">Добавить параграф</add-item-button>
              </div>
            ...
            <add-item-button @click.prevent="AddContentItem" title = "Добавить главу">Добавить главу</add-item-button>
          </div>
          ...
          <div class = "editing__sources">
            ...
            <add-item-button @click.prevent="AddSource" title = "Добавить источник">Добавить источник</add-item-button>
          </div>


          <div class = "editing__control">
            <site-button v-if="!isEdit" class="editing__button-add" @click.prevent="AddArticle" title = "Добавить статью">Добавить</site-button>
            <div class = "button_control" v-else>
              <site-button @click.prevent="UpdateArticle" title = "Сохранить статью">Сохранить</site-button>
              <site-button :isDelete="true" @click.prevent="ShowModal" title = "Удалить статью">Удалить</site-button>
            </div>
    ...
    <site-modal v-if="isDelete">
        <modal-delete @close="CloseModal" @execute="DeleteArticle" />
    </site-modal>
...
export default {
    ...
    methods:{
      ...
      async AddArticle(){
        ...
        if(article.info.title.length > 50){
          this.error = 'Заголовок статьи не должен превышать 50 символов'
          return
        }
        if(!article.info.description){
          this.error = 'Статья должна иметь описание'
          return
        }
        if(article.info.description.length > 50){
          this.error = 'Описание статьи не должно превышать 50 символов'
          return
        }
        if(!hasTag){
        ...
        if(!article.info.preview){
          this.error = 'Статья должна иметь превью'
          return
        }
        var objRE = /(^https?:\/\/)?[a-z0-9~_\-\.]+\.[a-z]{2,9}(\/|:|\?[!-~]*)?$/i;
        if(!objRE.test(article.info.preview)){
          this.error = 'Строка превью не является url-ссылкой'
          return
        }
        if(article.content.length == 0){
          this.error = 'Статья должна иметь наполнение'
          return
        }
        let index = 0
        for(let item of article.content){
          if(index != 0 && (item.title == null || item.title == "")){
            this.error = 'Блок ' + (index + 1) + ': ' + 'Глава должна иметь заголовок'
            return
          }
          if(item.paragraphs && !item.paragraphs.length){
            this.error = 'Блок ' + (index + 1) + ': ' + 'Глава должна иметь текст'
            return
          }
          for(let paragraph of item.paragraphs){
            if(paragraph.body == null || paragraph.body == ""){
              this.error = 'Блок ' + (index + 1) + ': ' + 'Каждый параграф должен иметь текст'
              return
            }
          }
          if((item.picture.desc || item.picture.copyrigth) && !item.picture.url){
            this.error = 'Блок ' + (index + 1) + ': ' + 'Добавьте изображение к главе либо удалите описание изображения'
            return
          }
          if(item.picture.url && !objRE.test(item.picture.url)){
            this.error = 'Блок ' + (index + 1) + ': ' + 'Строка не является url-ссылкой'
            return
          }
          index += 1
        }
        for(let source of article.sources){
        ...
        this.Expand(this.article).then(async res => {
          const url = (this.URL.API + `/articles`)
      ...
      async UpdateArticle(){
        ...
        this.Expand(this.article).then(async res => {
          const url = (this.URL.API + `/articles/` + article_id)
...
<style>
    ...
    .general-information__input-file_label:hover {
        background: v-bind("colors.accent ? colors.accent: '#DCA600'");
    }
    .general-information__input-file_label.disabled{
      background: #A5A5A5;
    }
    .general-information__input-file_label:hover.disabled:hover{
        background: #A5A5A5;
    }
    .editing__general-information{
      display: flex;
